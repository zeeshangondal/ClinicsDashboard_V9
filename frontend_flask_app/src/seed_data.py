#!/usr/bin/env python3
"""
Script to seed demo data for calls with different types
"""
...
def create_demo_calls():
    """Create demo calls with various types for testing"""
        ...
        try:
            ...
            for i in range(50):  # Create 50 demo calls
                ...
                call = Call(
                    ...
                    ai_summary=f"Demo {call_type.lower()} call with {contact_name}" if status == "completed" else None
                )
                
                db.session.add(call)
            
            db.session.commit()
            print(f"✅ Created 50 demo calls with various types")
